...
class SEL {
	public:
		...
		void set_equacoes(std::vector<std::pair<std::vector<double>, double>> _equacoes);
		...
		std::vector<double> calcular_residuo(std::vector<double> _estimativa);
...
#warning TODO: Implementar metodo da fatoracao LU
		...
		std::vector<double> gauss_jacobi(double _erro);
		std::vector<double> gauss_jacobi(std::vector<double> _inicial, double _erro);
		// Resolve o sistema pelo metodo de Gauss Seidel
		std::vector<double> gauss_seidel(double _erro);
		std::vector<double> gauss_seidel(std::vector<double> _inicial, double _erro);
	...
	private:
		...
		void abaixar_zeros(size_t _inicio);
		// Faz com que equacoes que possuam coeficientes maiores aparecam primeiro
		void dominantes(size_t _inicio);
		// Soma a linha1 com um multiplo da linha2
		// _linha1 += _m * _linha2
		void soma_linhas(size_t _linha1, size_t _linha2, double _m);
